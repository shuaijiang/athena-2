...
class AttentionDecoder(ABC):
    ...
    def build_loop_step(self):
        ...
        def loop_step(ts, eos_emitted, curr_state, logits,
            ...
            new_logits, new_state, new_alpha = self.step(
                decoder_input, curr_state, curr_alpha, 0)
    ...
    def build_beam_search_loop(self, beam_size=4, batch_size=16):
        ...
        def loop_batch(batch_index, stop_batch):
            ...
            # LM rescore n-best
            self.decoding_op = tf.identity(completed_seqs, name='decoding_output')
            self.decoding_op2 = tf.identity(completed_scores, name='decoding_scores')
            self.decoding_op3 = tf.identity(completed_scores_scaled, name='decoding_scores_scaled')
            ...
            return batch_index, stop_batch

        batch_index = tf.constant(0)
        stop_batch= tf.constant(False)
        batch_index, stop_batch = \
                    tf.while_loop(condition_bacth, loop_batch,[batch_index,stop_batch],
                                  parallel_iterations=1, back_prop=False, swap_memory=True)

    def build_batch_beam_search_loop(self, beam_size=4, batch_size=16):
        """Build Batch Beam Search decoding graph

        NOTE: 1. ONLY compatable with LAS for now!!
        """
        print("Build Batch Beam Search Decoding Graph")
        print("  beam size: {}".format(beam_size))
        print("  batch size: {}".format(batch_size))
        ...
        batch_index, ts, stop_decoder, states, alphas, cand_seqs, \
            ...
            completed_seqs, completed_lens = tf.while_loop(
            condition, loop_step,
            [ts, stop_decoder, states, alphas, cand_seqs,
             cand_scores, completed_scores, completed_scores_scaled,
             completed_seqs, completed_lens],
            parallel_iterations=1, back_prop=False, swap_memory=True)
